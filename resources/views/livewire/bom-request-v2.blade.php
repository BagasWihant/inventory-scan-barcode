    ...
    x-data="{
        ...
        get filteredData() {
            ...
            return this.listData.filter(r => 
                r.material_no.toLowerCase().includes(s) || 
                r.product_no.toLowerCase().includes(s) ||
                r.matl_nm.toLowerCase().includes(s)
            );
        },
        ...
        fetchData() {
            if(!this.lineCode || !this.date) return;
            this.isLoading = true;
            this.$wire.showData(this.lineCode, this.date);
        },
...
>
    <div class="flex gap-4 bg-white p-4 rounded-lg shadow-sm mb-6 border">
        <div class="flex-1">
            ...
            <input type="date" x-model="date" @change="$wire.getLineCode()" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 px-3 py-2">
        </div>
        <div class="flex-1">
            <label class="block text-xs font-bold text-gray-500 mb-1">Line Code</label>
            <select x-model="lineCode" @change="fetchData()" class="w-full border-gray-300 rounded-md shadow-sm px-3 py-2">
                <option value="">-- Select Line --</option>
                <template x-for="l in lines" :key="l.line_id">
                    <option :value="l.line_id" x-text="l.Line"></option>
                </template>
            </select>
        </div>
        <template x-if="pagedData.length > 0">
            <div class="flex-1">
                <label class="block text-xs font-bold text-gray-500 mb-1">Cari Material</label>
                <input type="text" x-model="search" placeholder="Cari..." class="w-full border-gray-300 rounded-md shadow-sm px-3 py-2">
            </div>
        </template>
    </div>
        ...
        <div class="p-4 border-b bg-gray-50 flex justify-between items-center text-sm">
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="flex border rounded overflow-hidden">
                    ...
                    <div class="px-4 py-1 bg-white flex items-center bg-gray-50 font-medium">
    ...
    <div x-show="selected.size > 0" class="fixed bottom-6 left-1/2 -translate-x-1/2">
        <button @click="submit()" class="bg-blue-600 text-white px-8 py-3 rounded-full shadow-2xl hover:bg-blue-700 font-bold flex items-center gap-3 transition-all hover:scale-105">
