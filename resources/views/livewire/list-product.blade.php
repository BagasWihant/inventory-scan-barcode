<div>
    <div class="mb-5">
        ...
        <input autofocus wire:model.live.debounce.300ms="search" type="text" id="large-input"
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @foreach ($productScanned as $product)
                        <tr @if($loop->index == 0 && $changes) wire:transition wire:key="{{ $product->id }}"  @endif
                            class="@if($loop->index == 0 && $changes) animasi-warna  @endif border rounded @if ($product->status == 1) bg-green-300 dark:bg-green-500 @else bg-white  dark:bg-gray-800 @endif dark:border-gray-700">
                            <th scope="row"
                                class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ $product->product_barcode }}</th>
                            <th scope="row"
